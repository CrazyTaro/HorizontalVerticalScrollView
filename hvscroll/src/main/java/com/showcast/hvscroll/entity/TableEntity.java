...
public class TableEntity {
    protected int mCellRowCount = 0;
    protected int mCellColumnCount = 0;
    protected int mMenuRowCount = 0;
    protected int mMenuColumnCount = 0;
...
//        Log.i("count", "row/count = " + table.getCellRowCount() + "/" + table.getCellColumnCount());
...
//        }
//        Log.i("count", "-------------------------------------\n");
//        Log.i("count", "before update\nrow/count = " + table.getCellRowCount() + "/" + table.getCellColumnCount());
//        table.autoUpdateRowColumnCount();
...
//        Log.i("count", "after update\nrow/count = " + table.getCellRowCount() + "/" + table.getCellColumnCount());
    ...
    /**
     * get the count of row or column by pointing out the lineType.<br>
     * 获取单元格行或列的数量
     *
     * @param lineType
     * @return
     */
    public int getCellCount(@Constant.LineType int lineType) {
        switch (lineType) {
            case Constant.LINE_ROW:
                return mCellRowCount;
            case Constant.LINE_COLUMN:
    ...
    public int getMenuCount(@Constant.MenuType int whichMenu) {
        switch (whichMenu) {
            case Constant.MENU_ROW:
                return mRowMenuList == null ? 0 : mMenuRowCount;
            case Constant.MENU_COLUMN:
                return mColumnMenuList == null ? 0 : mMenuColumnCount;
            default:
                return 0;
        }
    }
    ...
    public boolean autoUpdateMenuCount() {
        ...
        for (int i = mCellRowCount - 1; i >= 0; i--) {
        ...
        for (int i = mCellColumnCount - 1; i >= 0; i--) {
            menu = mColumnMenuList.get(i);
            if (menu != null) {
                newColumnCount = i;
            }
        }
        isChanged = newRowCount != mMenuRowCount || newColumnCount != mMenuColumnCount;
        mMenuRowCount = newRowCount + 1;
        mMenuColumnCount = newColumnCount + 1;
        return isChanged;
    }
    ...
    public boolean autoUpdateRowColumnCount() {
        ...
        for (int i = mCellRowCount - 1; i >= 0; i--) {
            ...
            if (newRowCount != mCellRowCount - 1) {
                if (row.size() > 0) {
            ...
            if (newColumnCount != mCellColumnCount - 1) {
                for (int j = mCellColumnCount - 1; j >= 0; j--) {
                    cell = row.get(j);
                    if (cell != null) {
        ...
        isChanged = mCellRowCount != newRowCount || mCellColumnCount != newColumnCount;
        mCellRowCount = newRowCount + 1;
        mCellColumnCount = newColumnCount + 1;
        return isChanged;
    }
    ...
    private void updateRowColumnCount(int rowCount, int columnCount) {
        mCellRowCount = mCellRowCount < rowCount ? rowCount : mCellRowCount;
        mCellColumnCount = mCellColumnCount < columnCount ? columnCount : mCellColumnCount;
    }
    ...
    private void updateMenuCount(int rowMenuCount, int columnMenuCount) {
        mMenuRowCount = mMenuRowCount < rowMenuCount ? rowMenuCount : mMenuRowCount;
        mMenuColumnCount = mMenuColumnCount < columnMenuCount ? columnMenuCount : mMenuColumnCount;
    }
